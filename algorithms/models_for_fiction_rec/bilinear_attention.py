...
class BilinearAttention(nn.Module):
    ...
    def score(self, query, key, softmax_dim=-1, mask=None, activation='softmax'):
        attn = self.matching(query, key, mask)
        print('sscore in Bilinear Att class')
        if activation == 'softmax':
            norm_attn = F.softmax(attn, dim=softmax_dim)
        elif activation == 'sigmoid':
            norm_attn = F.sigmoid(attn)
        else:
    ...
    def matching(self, query, key, mask=None):
        ...
        '''
        print('matching in Bilinear Att class')
    ...
    def forward(self, query, key, value, mask=None):
        ...
        '''
        print('forward in Bilinear Att class')
