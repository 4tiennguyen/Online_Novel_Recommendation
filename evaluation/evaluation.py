...
def evaluate_sessions(pr, metrics, test_data, train_data, items=None, cut_off=20, session_key='SessionId', item_key='ItemId', time_key='Time'):
    ...
    prev_iid, prev_sid = -1, -1  ## iid denotes item id, sid denotes session id
    pos = 0
    ...
    for i in range(len(test_data)):
        ...
        # todo 这里可以限制需不需要为这个item预测（评估） by yuncongli
        iid = test_data[item_key].values[i]
        ts = test_data[time_key].values[i]
        if prev_sid != sid:  # session第一个item不预测
            prev_sid = sid
            pos = 0
        else:
            if items is not None:
...
#             preds += 1e-8 * np.random.rand(len(preds)) #Breaking up ties
            ...
            if 'DataType' in test_data:
                data_type = test_data['DataType'].values[i]
                # {
                #         'feature': 1,
                #         'training': 2,
                #         'dev': 3,
