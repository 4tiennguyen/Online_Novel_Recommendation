...
def load_data_session(path, file, sessions_train=None, sessions_test=None, slice_num=None, train_eval=False,
                      train_full_train_valid_test=False):
    '''
    Loads a tuple of training and test set with the given parameters.
    ...
    train_tr = pd.read_csv(path + file + train_appendix + split + '.txt', sep='\t', dtype={'ItemId': np.int64})
    valid = pd.read_csv(path + file + test_appendix + split + '.txt', sep='\t', dtype={'ItemId': np.int64})

    train_tr, valid = prepare_data_session(train_tr, valid, sessions_train, sessions_test)

    print('END load data ', (time.perf_counter() - sc), 'c / ', (time.time() - st), 's')
    if train_full_train_valid_test:
        return train_tr, valid, test, train
    else:
        if train_eval:
            return train_tr, valid
        else:
            return train, test
