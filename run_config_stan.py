...
from helper.write_message import write
...
def main(conf, out=None):
    ...
    if file.is_dir():
        ...
        if out is not None:
            ...
            write('waiting for configuration files in ' + conf)
            ...
            while True:
                ...
                if len(list) > 0:
                    try:
                        ...
                        write('processing config' + list[0])
...
def run_single(conf, slice=None):
    ...
    train, valid, test, train_full = None, None, None, None
    if 'type' in conf['data']:
        ...
        if 'opts' in conf['data']:
            if 'train_full_train_valid_test' in conf['data']['opts'] \
                    and conf['data']['opts']['train_full_train_valid_test']:
                train, valid, test, train_full = dl.load_data_session(conf['data']['folder'], conf['data']['prefix'],
                                                                      slice_num=slice, **conf['data']['opts'])
            else:
    ...
    # for other baselines
    # baseline_data_dir = '/Users/liyuncong/PycharmProjects/session-rec-pytorch-private/data/' \
    #                     'fiction-with-attributes-for-baselines/prepared/'
    # train.to_csv(baseline_data_dir + '/train.txt', sep='\t', index=False)
    # valid.to_csv(baseline_data_dir + '/valid.txt', sep='\t', index=False)
    # test.to_csv(baseline_data_dir + '/test.txt', sep='\t', index=False)


    for k, a in algorithms.items():
        eval_algorithm(train, test, k, a, evaluation, metrics, results, conf, slice=slice, iteration=slice,
                       valid=valid, train_full=train_full)
...
def run_opt_single(conf, iteration, globals):
    '''
    这里优化的超参不包含神经网络的epoch，所以训练神经网络时，early stopping还是需要的 by yuncongli
    Evaluate the algorithms for a single split
        --------
        conf: dict
            Configuration dictionary
...
def eval_algorithm(train, test, key, algorithm, eval, metrics, results, conf, slice=None, iteration=None, out=True,
    ...
    if 'train_eval' in conf['data'] and conf['data']['train_eval']:
        valid = test

    if hasattr(algorithm, 'init'):
        algorithm.init(train, valid, slice=slice)
    ...
    # 创建针对小说推荐的模型目录
    common_path.repeat_aware_rec_base_dir = os.path.join(common_path.repeat_aware_rec_base_dir, key) + '/'
    os.makedirs(common_path.repeat_aware_rec_base_dir, exist_ok=True)

    # 训练的时候，是不会需要测试集的 by yuncongli
    write('fit')
    algorithm.fit(train, valid)
    ...
    write('evaluate')
    if 'evaluation_by_algorithm_self' in conf and conf['evaluation_by_algorithm_self']:
        print('Eval Eval Eval')
        algorithm.eval(valid, test)
    else:
    ...
    #print('-------------Result: ')
    #print(results)
    #print('--type: ', type(results[key][-4][2]))
    #print('--------------------')
    #print('---------------------')
    #print(results[key][-4][2])
    results[key][-4][2].to_csv('stan_06.csv', sep='\t', encoding='utf-8')

    #results[key][-4][2].to_csv('C:/Users/tttie/OneDrive/Desktop/CS 274/NovelNet-main/rar_base_dir/ar/ar.csv', sep='\t', encoding='utf-8')
    ...
    for algorithm in list:
        ...
        if 'params' in algorithm:
            if 'algorithms' in algorithm['params']:
                ...
                for k, val in params.items():
                    if k not in ('file', 'train', 'evaluation', 'infer', 'other_args', 'features') \
                            and not k.startswith('analysis'):
                        key += '-' + str(k) + "=" + str(val)
                        key = key.replace(',', '_')
